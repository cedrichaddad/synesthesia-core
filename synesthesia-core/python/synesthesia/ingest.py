...
def ingest_ark(csv_path: str, vector_engine: VectorEngine, batch_size: int = 1000):
    """
    Ingest the Ark (tracks_features.csv) into Qdrant.
    """
    if not os.path.exists(csv_path):
        print(f"Ark file not found: {csv_path}")
        return

    print(f"Opening Ark: {csv_path}")
    ...
    try:
            ...
            for row in reader:
                try:
                    ...
                    track = {
                        ...
                        'name': row.get('name'),
                        'artist': artist,
                        'genre': 'Unknown', # CSV doesn't have genre, maybe infer or leave unknown
                        'energy': energy,
                        'valence': valence,
                    ...
                    if len(batch_tracks) >= batch_size:
                        vector_engine.upsert_batch(batch_tracks, batch_vectors)
        ...
        print(f"Ark Ingestion Complete. Total Tracks: {count}")
